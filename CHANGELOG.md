...
## Improvements
- Removed the last Tkinter dependency: the Neurophotometrics Step 1 prompts (split-events and timestamp column/unit) are now rendered on the Label Stores page as Panel widgets with a "Confirm NPM configuration" button, so GuPPy is Panel-only. [PR #XXX](https://github.com/LernerLab/GuPPy/pull/XXX)
