...
## Features
- Added support for reading pyPhotometry `.ppd` recordings, the open acquisition hardware from Akam and Walton (2019) also sold by Open Ephys. A session folder holding one `.ppd` file is detected like any other format and its analog inputs and digital lines appear as stores in Label Stores, so the rest of the pipeline is unchanged. Every header generation the format has had is read, including the laboratory fork whose analog inputs each time-multiplex two colours; an acquisition mode the reader does not know is refused rather than read with the default two-input layout, which would silently interleave several colours into one trace. Because the board has no simultaneous analog-to-digital converters, the timestamps of a strobed recording's inputs are staggered by one tick of the sampling timer, as the hardware sampled them. [PR #447](https://github.com/LernerLab/GuPPy/pull/447)
