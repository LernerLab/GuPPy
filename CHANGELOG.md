...
## Fixes
- Neurophotometrics recordings are now demultiplexed by reading each frame's `LedState`/`Flags` word as the packed value it is, testing the excitation bit that selects a channel rather than striding through the file from the first row carrying a given value. A recording that writes one excitation under several values — `sampleData_NPM_4` writes 415 nm as both 17 and 273, either side of a digital input going high — or that strobes two LEDs into a single frame is read correctly as a result, and a dropped frame no longer misassigns every sample after it. A file whose header carries blank column titles, or a second timestamp column alongside one named `Timestamp`, now opens the Label Stores page instead of leaving it blank with a pandas error visible only in the terminal running the server; a `FrameCounter` column is no longer required, and any other problem with an NPM configuration is reported in the page's alert pane rather than reaching only the terminal running the server, on saving as well as on confirming. A channel landing on one more frame than the file's first channel is no longer written with more data points than it has timestamps. A session whose files interleave different sets of LEDs is now read instead of refused: channels are paired within the file they came from, so each file's channels take that file's own first-channel clock, where before they were paired by position across files and a session whose files disagreed was rejected outright. [PR #507](https://github.com/LernerLab/GuPPy/pull/507)
...
## Improvements
- Neurophotometrics channels are now named after the source file, excitation wavelength and region column they were demultiplexed from — `signals_415nm_G2` in place of `file0_chev3` — so a store list says what each channel is without a lookup. Only region columns become stores, so the `Stimulation`, `Output0/1` and `Input0/1` digital lines that files from one 2021 firmware window carry are no longer demultiplexed into channels of their own; `sampleData_NPM_2` drops from fourteen stores to four as a result. Header-less recordings name no LED, so their channels keep the positional `chev`/`chod`/`chpr` slots and take only the file's name as a prefix. A `storesList.csv` saved by an earlier run of an NPM session names the old identifiers and will need re-labeling. `.npm_params.json` now also records, under `stores`, what each channel was demultiplexed from — the source file, the excitation wavelength, the data column, and the timestamp column it was timed by — since an NPM store name is invented during demultiplexing and a run folder that records only the names leaves a tool reading it to reproduce that demultiplexing to work out what they refer to. The clock is part of that record because it is resolved per file rather than per session: a file offering a single timestamp column reads that one whatever `npm_timestamp_column_name` names. [PR #507](https://github.com/LernerLab/GuPPy/pull/507)
