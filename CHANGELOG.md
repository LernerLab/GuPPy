...
## Features
- Added export to NWB, with a metadata input form for supplying subject/session details. Every acquisition format GuPPy reads raw files for is supported — TDT, Doric, Neurophotometrics and GuPPy's own CSV layout — and a session's format is detected from its folder rather than assumed. The metadata form takes a session start time, marked as required for every format but TDT, whose tank header is the only one that reliably records one. Sessions the export cannot take are refused upfront with a message naming why: sessions read from an NWB file (locally or streamed from DANDI), sessions whose folder holds more than one acquisition format, and sessions analyzed with **Combine Data?** enabled. NWB export also aborts upfront with a clear message when a selected session had its artifacts removed with the `concatenate` method, which re-times samples and breaks alignment to the acquisition clock. [PR #357](https://github.com/LernerLab/GuPPy/pull/357)
