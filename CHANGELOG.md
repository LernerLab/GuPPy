...
## Features
- Analysis outputs no longer land inside the session folder. You now name a **data root** in the Input Folder Selection card and an **output directory** in Output Folder Selection, and GuPPy mirrors each session's path below the data root into the output directory, writing its run folders there as `output_<run name>` — so a session at `<data root>/subject1/session1` gets `<output directory>/subject1/session1/output_1`. Mirroring the input structure means the results tree is navigable without documentation and two sessions sharing a folder name never collide. Both directories are required; GuPPy refuses to start an analysis rather than choosing a location on your behalf. The previous layout remains available as the **Output Location** setting *write each run inside the session folder it came from*. [PR #508](https://github.com/LernerLab/GuPPy/pull/508)
...
## Improvements
- The data root and the output directory can now be named at launch with `guppy --data-root <path> --output-directory <path>`, so the two directories that rarely change between analyses need not be picked again each session. [PR #508](https://github.com/LernerLab/GuPPy/pull/508)
