...
## Features
- The DANDI source panel now browses the archive over three screens: a list of dandisets, one dandiset's metadata, and that dandiset's NWB files. It opens having already searched for fiber photometry datasets, and the same search box takes any other term or a six-digit Dandiset ID. Because DANDI records nothing about fiber photometry in its structured metadata, that search matches only what a dataset's authors wrote about it; a checkbox reads the listed dandisets' files instead and keeps only those GuPPy can actually read, and the file browser does the same for a chosen dandiset's assets. Selecting a file previews its channels, their recording sites, indicators and wavelengths, the store names Step 1 will ask you to label, and the first 60 seconds of every channel. [PR #506](https://github.com/LernerLab/GuPPy/pull/506)
