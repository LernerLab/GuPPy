...
## Fixes
- Cross-correlation between two brain regions no longer crashes when independent artifact removal leaves the regions with a different number of trials (or trials of different sample lengths): the two regions are now aligned to their common trial count and sample length, and the trial columns are labeled to match, so a per-region timestamp mismatch produces a valid result with a warning instead of a `ValueError` that forced an application restart. [PR #388](https://github.com/LernerLab/GuPPy/pull/388)
