...
## Features
- Added Iteratively Re-Weighted Least Squares (IRWLS) as the control-channel fitting method and made it the new default (robust to outliers; ordinary least-squares `OLS` fitting remains selectable via the new `control_fit_method` parameter). Resolves [#199](https://github.com/LernerLab/GuPPy/issues/199).
