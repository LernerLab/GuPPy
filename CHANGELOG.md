...
## Features
- GuPPy now remembers the input and output root folders between launches and pre-selects them, so the **Root Folder Selection** card is folded away for a returning user and only the session folders are left to pick. The pair is recorded when an analysis starts rather than while browsing, and `--input-root` / `--output-root` still win over it. [PR #515](https://github.com/LernerLab/GuPPy/pull/515)
- `guppy --input-root <path> --output-root <path>` names the two root folders at launch and pre-selects them in the **Root Folder Selection** card, which folds away once both are known. These replace `--start-path`: the input root already decides where the session browser opens. [PR #514](https://github.com/LernerLab/GuPPy/pull/514)
- Analysis outputs no longer land inside the session folder. A new **Root Folder Selection** card names two folders a project keeps for the life of its analyses — an **input root folder** your sessions live under and an **output root folder** results are written to — and GuPPy mirrors each session's path below the input root into the output root, writing its run folders there as `output_<run name>`. A session at `<input root>/subject1/session1` gets `<output root>/subject1/session1/output_1`. Mirroring the input structure means the results tree is navigable without documentation and two sessions sharing a folder name never collide. Both folders are required. Ticking **Output root folder is the same as the input root folder** points both at one folder, which writes each session's runs inside it — the pre-2.0.0-beta4 arrangement, available but not recommended. Run folders written by earlier versions are still found and still open, and occupy their run name so a new run never collides with one; reading that older name is deprecated. [PR #513](https://github.com/LernerLab/GuPPy/pull/513)
...
## Fixes
- DANDI mode no longer shows the **Session folders** instruction above the DANDI browser, which told you to pick sessions under an input root folder that the mode does not have. [PR #516](https://github.com/LernerLab/GuPPy/pull/516)
