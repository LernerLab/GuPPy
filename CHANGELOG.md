...
## Features
- Added tonic/basal fluorescence analysis for pharmacological experiments as a new optional **Define Tonic Epochs** step: name epoch windows (e.g. baseline vs. post-injection) per recording site on the preprocessed traces, and saving averages the z-score and ΔF/F over each window into `tonic_<recording_site>.h5`. The visualization's Tonic tab charts the per-epoch means as bars against a selectable baseline epoch, alongside the windows shaded on the traces and a table of the same numbers. PSTH analysis is unaffected. [PR #397](https://github.com/LernerLab/GuPPy/pull/397)
