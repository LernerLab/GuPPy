...
## Improvements
- The artifact-marking table no longer redraws all three traces on every cell edit, and it gained add/remove-row controls and an "apply to all recording sites" button in place of its fixed three rows and unused label column. [PR #414](https://github.com/LernerLab/GuPPy/pull/414)
...
- Removed the `removeArtifacts?` and `removeArtifacts method` controls from the Input Parameters form; the method is now chosen on the Select Artifact Windows page, and both keys remain in `GuPPyParamtersUsed.json` as a record of what was applied to each run. [PR #414](https://github.com/LernerLab/GuPPy/pull/414)
