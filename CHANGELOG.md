...
## Features
- Artifact removal is now two optional steps of its own — **Select Artifact Windows** and **Remove Artifacts** — so a session no longer has to be preprocessed twice with a checkbox flipped in between; you mark the periods where artifacts occurred (rather than the chunks to keep) and apply them in one pass. [PR #413](https://github.com/LernerLab/GuPPy/pull/413)
...
## Improvements
- Artifact periods are now marked with form controls — one row of two numeric bounds and a delete button per period, nudgeable with the arrow keys — instead of an editable table with a fixed three rows and an unused label column; editing a bound repaints only the shaded spans rather than redrawing all three traces, and an "apply to all recording sites" button copies a period across sites. [PR #413](https://github.com/LernerLab/GuPPy/pull/413)
...
- Removed the `removeArtifacts?` and `removeArtifacts method` controls from the Input Parameters form; the method is now chosen on the Select Artifact Windows page, and both keys remain in `GuPPyParamtersUsed.json` as a record of what was applied to each run. [PR #413](https://github.com/LernerLab/GuPPy/pull/413)
