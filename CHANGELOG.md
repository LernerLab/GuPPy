...
## Improvements
- Choosing which existing run Steps 2–5 read is now done by run name rather than by browsing to an output directory: one **Run name(s) for all sessions** choice covers the whole batch, and a per-session picker underneath shows what each session will use and can be changed on its own. Changing the session selection no longer discards the run choices already made for the other sessions. [PR #474](https://github.com/LernerLab/GuPPy/pull/474)
