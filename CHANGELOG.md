...
## Fixes
- Doric CSV timestamps are now on the absolute recording clock, matching the `.doric` V1/V6 paths: the CSV reader no longer re-zeros `Time(s)` to start at 0, so both the continuous signal/control streams and the TTL event onsets keep the acquisition clock. [PR #XXX](https://github.com/LernerLab/GuPPy/pull/XXX)
