...
## Fixes
- The main GUI no longer freezes with a "server connection lost" error while a pipeline step runs: the progress bar now polls from a periodic callback on the server event loop instead of blocking it with a busy-wait and thread join, so the app keeps answering the browser throughout long or interactive steps. [PR #401](https://github.com/LernerLab/GuPPy/pull/401)
...
## Improvements
- Migrated the remaining matplotlib pop-up windows to Panel: the Step 3 preprocessing/artifact-removal and Step 4 transient-peak plots now render as in-browser Panel pages, and artifact boundaries are marked by typing good-chunk windows in an editable per-recording-site table (shaded live on the traces) instead of clicking on a native matplotlib figure. [PR #401](https://github.com/LernerLab/GuPPy/pull/401)
