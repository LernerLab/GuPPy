...
## Improvements
- Added a [Design decisions](https://guppy.readthedocs.io/en/latest/contributing/design_decisions.html) page to the Contributor's Guide, covering the rationale behind choices the [Architecture](https://guppy.readthedocs.io/en/latest/contributing/architecture.html) page leaves out: HDF5 as the inter-step medium, background threads vs. the Panel IOLoop, `spawn` vs. `fork` for multiprocessing, pull-based progress reporting, the two server-lifetime strategies for step views, the `inputParameters` dict contract, the headless testing API's design principle, validation layering, and run folders as the unit of output. [PR #433](https://github.com/LernerLab/GuPPy/pull/433)
