...
## Features
- **Timestamps for Each Control/Signal Pair** (`pair_timestamps_channel`) chooses whose sample times a control/signal pair is analyzed on, for systems that sample the two channels at different moments: `signal` (the default) or `control`. The longer channel is trimmed to the length of the shorter. [PR #523](https://github.com/LernerLab/GuPPy/pull/523)
...
## Fixes
- Each Neurophotometrics channel is now timed by the frames its own LED lit, rather than by its file's first channel, which labeled a 470 nm sample with the timestamp of the 415 nm frame beside it and dropped a channel's last sample when it landed on one more frame. [PR #527](https://github.com/LernerLab/GuPPy/pull/527)
- Neurophotometrics recordings are now demultiplexed by testing the excitation bit of each frame's `LedState`/`Flags` word, so a recording that writes one excitation under several values — `sampleData_NPM_4` writes 415 nm as both 17 and 273 — or strobes two LEDs into one frame is read correctly, and a dropped frame no longer misassigns every sample after it. A file whose header carries blank cells, or a second timestamp column beside one named `Timestamp`, now opens the Label Stores page, and a problem with an NPM configuration is reported in the page's alert pane rather than only in the terminal running the server. [PR #526](https://github.com/LernerLab/GuPPy/pull/526)
...
## Improvements
- The Neurophotometrics consistency tests compare against the unmodified v1.3.0 references, running Step 3 on the control's clock and trimming the one frame `sampleData_NPM_4`'s 470 nm store now keeps. [PR #529](https://github.com/LernerLab/GuPPy/pull/529)
- The Label Stores page now asks whether to split each Neurophotometrics event file by the file's name, and `npm_split_events` is recorded per event file name rather than by position in the session's file list. A run folder whose `.npm_params.json` records split events by position needs Step 1 re-run. [PR #528](https://github.com/LernerLab/GuPPy/pull/528)
- Neurophotometrics channels are now named after the source file, excitation wavelength and region column they come from — `signals_470nm_G2` in place of `file0_chod3` — and only region columns become stores, so the `Stimulation`, `Output0/1` and `Input0/1` digital lines are no longer channels of their own. A `storesList.csv` saved by an earlier run of an NPM session names the old identifiers and will need re-labeling. `.npm_params.json` now records, under `stores`, the file, excitation wavelength or interleave position, data column and timestamp column each channel was read from. [PR #526](https://github.com/LernerLab/GuPPy/pull/526)
