...
:::{note}
There is no separate "save parameters" action. Each pipeline step records the parameters it ran with into its output folder as `GuPPyParamtersUsed.json`, so that file always reflects the configuration that was actually applied. If you come back later, reselect the run's name under **Output Folder Selection**, and GuPPy reloads that run's saved parameters into the form so you can continue (for example, run Preprocess and PSTH) without accidentally overwriting them.
:::
