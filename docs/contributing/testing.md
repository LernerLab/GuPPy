...
`build_homepage()` to assemble the Panel template, point its Root Folder Selection card at the
test's data directory, and then drive the pipeline through `step1()` through `step5()` from
