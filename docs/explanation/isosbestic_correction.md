...
[Fiber photometry](fiber_photometry.md) uses GCaMP because its fluorescence tracks calcium concentration: more calcium, more emission. What is less obvious is that *how much* the fluorescence changes depends on the laser wavelength used to excite the indicator. At some wavelengths a calcium event produces a large change, at others a smaller one, and at one specific wavelength, the **isosbestic point**, the response is zero (calcium concentration changes produce no fluorescence change at all):
...
![The full GuPPy isosbestic correction on a 60 s synthetic trace. Faint vertical lines mark calcium-event times (green, t = 10, 30, 50 s) and motion-artifact times (amber, t = 20, 42 s) so each event can be traced top-to-bottom. (1) The raw 470 nm signal (green) and 405 nm control (purple) are recorded simultaneously and share a slow bleach plus two motion artifacts; calcium events appear only on the 470 channel. (2) The linear fit (dashed grey) is overlaid on the signal; rescaling the control onto the signal's baseline is what makes subtraction sensible despite the two channels being on different absolute scales. (3) The residual (signal minus fit) isolates what the rescaling cannot explain: the calcium events plus a little noise. (4) The dF/F: same residual divided by the fitted control and expressed as a percentage. The shared slow drift and motion bumps cancel; the three calcium events remain.](../_static/images/isosbestic_explainer/fig3_linear_fit_and_correction.svg)
...
![A wavelength-dependent artifact illustrated on a 20 s synthetic recording. Left: raw signal (green), raw 405 control (purple), and the rescaled control overlaid as a thin gray dashed line. The rescaled control hugs the signal almost everywhere, but at t = 10 s the green signal dips much further than the gray dashed: the gap between them is exactly what the correction cannot cancel. Right: the corrected dF/F. The surviving spike at t = 10 s is comparable in magnitude to a real calcium event (dashed reference at ~10%, the typical event amplitude from the previous figure), which is why a wavelength-dependent artifact can be mistaken for neural activity.](../_static/images/isosbestic_explainer/fig4_what_survives.svg)
...
![Same 60 s synthetic recording corrected two ways. Top row: the input signal (green) is identical in both columns; the control used for the fit is either the real isosbestic trace (left, purple) or the synthetic exponential built from the signal itself (right, dashed red). Bottom row: the resulting dF/F. Three calcium events at t = 10, 30, 50 s appear in both bottom panels. The motion artifact at t = 20 s (amber band) cancels under the real isosbestic correction (left) but passes through under the synthetic exponential (right), where it appears as a fourth peak in the dF/F that is indistinguishable in shape from a real calcium event.](../_static/images/isosbestic_explainer/fig6_synthetic_fallback.svg)
