...
class NpmRecordingExtractor(BaseRecordingExtractor):
    ...
    def read(self, *, events: list[str], outputPath: str) -> list[dict[str, Any]]:
        ...
        divisor = TIME_UNIT_DIVISORS[self.npm_time_unit or "seconds"]
    ...
    def stub(self, *, folder_path: str | Path, duration_in_seconds: float = 1.0) -> None:
        ...
        Each NPM CSV is cut at its own first timestamp plus ``duration_in_seconds``, read on
        the file's first timestamp column (its leading column when it has no header). A first
        timestamp above ``1e6`` is taken to be in milliseconds.
    ...
    def _read_source_files(cls, folder_path: str | Path) -> dict[str, pd.DataFrame]:
        ...
        A file whose first row is numeric has no header, and its columns are labeled by
        position.
        ...
        source_paths = [
            path for path in sorted(set(session_folder.glob("*.csv")) - derived_paths) if not _is_event_csv(path)
        ]
        dataframes = {}
