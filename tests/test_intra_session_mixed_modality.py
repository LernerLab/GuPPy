...
@pytest.mark.filterwarnings("ignore::UserWarning")
def test_mixed_modality_npm_csv_ttl_raises(tmp_path, monkeypatch):
    """
    Intra-session NPM + CSV TTL raises ValueError (NPM caveat documented).

    When NPM data files are present in a session folder, detect_acquisition_formats suppresses
    the 'csv' format to avoid confusing NPM split-event files with external TTL CSVs.
    As a result, a synthesized CSV event file is not routed to any extractor, and step3 raises
    ValueError when it cannot find the event in the event-to-extractor mapping.

    This test documents the current limitation: external CSV TTL events cannot be combined
    with NPM photometry data within a single session folder.
    """
    src_base_dir = str(Path(".") / "testing_data")
    tmp_base = tmp_path / "data_root"
    tmp_base.mkdir(parents=True, exist_ok=True)
    ...
    # Synthesize a CSV event file inside the NPM session folder
    csv_ttl_timestamps = np.array([49900000.0, 49910000.0, 49920000.0])
    np.savetxt(session_copy / "csv_npm_event.csv", csv_ttl_timestamps, header="timestamps", comments="", fmt="%.6f")
    ...
    step2(
        ...
        storenames_map={
            ...
            "csv_npm_event": "ttl_region",
        },
        npm_split_events=[False, True],
    )

    with pytest.raises(ValueError, match="csv_npm_event"):
        step3(
            base_dir=base_dir,
            selected_folders=selected_folders,
            npm_split_events=[False, True],
