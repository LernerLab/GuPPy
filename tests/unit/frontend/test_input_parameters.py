...
class TestRootFolderSelection:
    ...
    def test_a_remembered_pair_is_pre_selected_at_the_next_launch(
        self, panel_extension, frontend_base_dir, tmp_path, output_root_folder
    ):
        remember_root_folders(input_root_folder=str(tmp_path), output_root_folder=str(output_root_folder))

        form = ParameterForm(template=pn.template.BootstrapTemplate(title="Test"), start_path=str(frontend_base_dir))

        assert form.input_root_folder == str(tmp_path)
        assert form.output_root_folder == str(output_root_folder)
        assert form.root_folder_selection.collapsed is True

    def test_the_command_line_wins_over_what_was_remembered(
        self, panel_extension, frontend_base_dir, tmp_path, output_root_folder
    ):
